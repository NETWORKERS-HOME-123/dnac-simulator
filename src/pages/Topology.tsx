...
export default function Topology() {
  ...
  const renderNode = (node: TopologyNode) => {
    ...
    return (
      ...
      >
        {config.shape === 'circle' && (
          <circle r={size} fill={config.color} stroke={isSelected ? 'white' : 'none'} strokeWidth={isSelected ? 3 : 0} opacity={0.9} />
        )}
        {config.shape === 'rect' && (
          <rect x={-size} y={-size} width={size * 2} height={size * 2} rx={4} fill={config.color} stroke={isSelected ? 'white' : 'none'} strokeWidth={isSelected ? 3 : 0} opacity={0.9} />
        )}
        {config.shape === 'triangle' && (
          <polygon points={`0,${-size} ${size},${size} ${-size},${size}`} fill={config.color} stroke={isSelected ? 'white' : 'none'} strokeWidth={isSelected ? 3 : 0} opacity={0.9} />
        )}
        {config.shape === 'diamond' && (
          <polygon points={`0,${-size} ${size},0 0,${size} ${-size},0`} fill={config.color} stroke={isSelected ? 'white' : 'none'} strokeWidth={isSelected ? 3 : 0} opacity={0.9} />
        )}
        <text y={size + 16} textAnchor="middle" fill="hsl(213, 27%, 30%)" fontSize={11} fontWeight={500}>
          {node.label}
        </text>
      </g>
    );
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 border rounded-md p-1 bg-card">
            {[
              ...
              <Button
                key={f.label}
                variant={filter === f.type ? "default" : "ghost"}
                size="sm"
          ...
          <div className="flex items-center gap-1 border rounded-md p-1 bg-card">
            <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => setZoom(z => Math.min(z + 0.15, 2))}>
              <ZoomIn className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => setZoom(z => Math.max(z - 0.15, 0.4))}>
      ...
      <div className="flex items-center gap-4 text-xs text-muted-foreground">
        ...
        <span className="ml-4 flex items-center gap-1.5">
          Link: <span className="w-6 h-0.5" style={{ backgroundColor: 'hsl(142, 71%, 45%)' }} /> Low
          <span className="w-6 h-0.5" style={{ backgroundColor: 'hsl(38, 92%, 50%)' }} /> Med
          <span className="w-6 h-0.5" style={{ backgroundColor: 'hsl(0, 72%, 51%)' }} /> High/Down
        </span>
      </div>
      ...
      <Card className="border-0 shadow-sm overflow-hidden">
        <CardContent className="p-0">
          <svg
            ...
            width="100%"
            height="620"
            className="topology-canvas bg-card"
                ...
                return (
                  <g key={link.id}>
                    <line
                      ...
                      strokeDasharray={link.status === 'down' ? '6,4' : 'none'}
                      opacity={0.7}
                    />
                    <text
                      x={(s.x + t.x) / 2}
              ...
              <SheetHeader>
                <SheetTitle className="flex items-center gap-2">
                  ...
                  <Badge
                    ...
                    style={{
                      backgroundColor: selectedDevice.healthScore >= 8 ? 'hsl(142, 71%, 90%)' : selectedDevice.healthScore >= 5 ? 'hsl(38, 92%, 90%)' : 'hsl(0, 72%, 90%)',
                      color: selectedDevice.healthScore >= 8 ? 'hsl(142, 71%, 30%)' : selectedDevice.healthScore >= 5 ? 'hsl(38, 92%, 30%)' : 'hsl(0, 72%, 30%)',
                    }}
                  >
                    Health: {selectedDevice.healthScore}/10
                  </Badge>
                </SheetTitle>
                <SheetDescription>{selectedDevice.platform}</SheetDescription>
              </SheetHeader>
